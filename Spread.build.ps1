...
task CreateHelp {
    ...
    New-ExternalHelp -Path "$BuildRoot\docs\en-US" -OutputPath "$BuildRoot\Spread\en-US" -Force
    Remove-Module Spread, platyPS
}

# Synopsis: Generate .\Release structure
task GenerateRelease CreateHelp, {
    # Setup
    if (-not (Test-Path "$releasePath\Spread")) {
        $null = New-Item -Path "$releasePath\Spread" -ItemType Directory
    }
    ...
    Copy-Item -Path $additionalFiles -Destination "$releasePath\Spread" -Force
}

# Synopsis: Update the manifest of the module
task UpdateManifest GetVersion, {
    Update-Metadata -Path "$releasePath\Spread\Spread.psd1" -PropertyName ModuleVersion -Value $script:Version
    # Update-Metadata -Path "$releasePath\Spread\Spread.psd1" -PropertyName FileList -Value (Get-ChildItem $releasePath\Spread -Recurse).Name
    $functionsToExport = Get-ChildItem "$BuildRoot\Spread\Public" | ForEach-Object {$_.BaseName}
    Set-ModuleFunctions -Name "$releasePath\Spread\Spread.psd1" -FunctionsToExport $functionsToExport
}

task GetVersion {
    $manifestContent = Get-Content -Path "$releasePath\Spread\Spread.psd1" -Raw
...
$ConvertMarkdown = @{
    Inputs  = { Get-ChildItem "$releasePath\Spread\*.md" -Recurse }
    Outputs = {process {
...
task Deploy -If ($env:APPVEYOR_REPO_BRANCH -eq 'master' -and (-not($env:APPVEYOR_PULL_REQUEST_NUMBER))) {
    Remove-Module Spread -ErrorAction SilentlyContinue
}, PublishToGallery

task PublishToGallery {
    assert ($env:PSGalleryAPIKey) "No key for the PSGallery"

    Import-Module $releasePath\Spread\Spread.psd1 -ErrorAction Stop
    Publish-Module -Name Spread -NuGetApiKey $env:PSGalleryAPIKey
}
...
task RemoveMarkdownFiles {
    Remove-Item "$releasePath\Spread\*.md" -Force -ErrorAction 0
}
# endregion
